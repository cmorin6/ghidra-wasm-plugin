...
import wasm.analysis.flow.InstructionFlowUtil.BrTable;
import wasm.analysis.flow.InstructionFlowUtil.BrTarget;
...
public class PcodeOpEmitter {
	...
	private AddressSpace globalSpace;
	...
	public PcodeOpEmitter(SleighLanguage language, Address opAddr, long uniqBase) {
		...
		globalSpace = language.getAddressFactory().getAddressSpace(GLOBAL);
	...
	public void emitBrTable(BrTable tbl) {
		...
		opList.add(new PcodeOp(opAddress, seqnum++, PcodeOp.COPY, new Varnode[] { spVarnode }, tmpSP));
		...
		for (int i = 0; i < tbl.numCases(); i++) {
			...
			Varnode subFromSP = getConstant((long) (target.getNumPops() * 8), 8); // this is how many bytes we should
																					// implicitly pop
			Varnode branchDest = getAddress(target.getDest());

			opList.add(new PcodeOp(opAddress, seqnum++, PcodeOp.INT_EQUAL, new Varnode[] { brIdx, constComparator },
					takeBranch));
			// takeBranch = brIdx == constComparator;
			opList.add(
					new PcodeOp(opAddress, seqnum++, PcodeOp.INT_SUB, new Varnode[] { tmpSP, subFromSP }, spVarnode));
			// SP = tmpSP - subFromSP;
			opList.add(new PcodeOp(opAddress, seqnum++, PcodeOp.CBRANCH, new Varnode[] { branchDest, takeBranch }));
		...
		opList.add(new PcodeOp(opAddress, seqnum++, PcodeOp.INT_SUB, new Varnode[] { tmpSP, subFromSP }, spVarnode));
		// SP = tmpSP - subFromSP;
		opList.add(new PcodeOp(opAddress, seqnum++, PcodeOp.BRANCH, new Varnode[] { defaultDest }));
		// goto defaultDest
	}
	...
	public void emitCall(WasmFuncSignature target, int extraLocalSaveIndex) {
	...
	public Varnode emitGetIndirectFuncAddr() {
		...
		opList.add(new PcodeOp(opAddress, seqnum++, PcodeOp.INT_MULT, new Varnode[] { v, getConstant(8, 4) }, off));

		Varnode res = findVarnode("tmpFuncAddr", 8);
		opList.add(new PcodeOp(opAddress, seqnum++, PcodeOp.LOAD,
				new Varnode[] { getConstant(tableSpace.getSpaceID(), 4), off }, res));

		return res;
	}

	public void emitCallIndirect(WasmFuncType type, int extraLocalSaveIndex) {
	...
	public void emitStackSetup(GlobalStack gs) {
		Varnode[] in;
		PcodeOp op;
		// [global]0:8 = SP
		in = new Varnode[3];
		in[0] = getConstant(globalSpace.getSpaceID(), 4);
		in[1] = getConstant(gs.getGlobalIndex(), spVarnode.getSize());
		in[2] = spVarnode;
		op = new PcodeOp(opAddress, seqnum++, PcodeOp.STORE, in);
		opList.add(op);
		...
		in = new Varnode[2];
		in[0] = spVarnode;
		in[1] = getConstant(gs.getStackSize(), spVarnode.getSize());
		// PcodeOp
